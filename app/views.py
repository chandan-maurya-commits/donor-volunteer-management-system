        ...
        try:
            ...
            if user:
                if user.is_staff:
                    ...
                    messages.warning(request,'Ivalid Admin User')
            else:
...
class login_donor(View):
    ...
    def post(self,request):
        ...
        try:
            user = authenticate(username=us, password=pwd)
            if user:
                donar_user = Donor.objects.filter(user_id=user.id)
                if donar_user:
                    login(request, user)
                    # messages.success(request,'Login Successfully')
...
def donationrec_admin(request):
    if not request.user.is_authenticated:
        return redirect('/login-admin')
    donation = Donation.objects.filter(status='Donation Recieved')
...
class view_volunteerdetail(View):
    def get(self,request,pid):
        ...
        return render(request, "view-volunteerdetail.html",{'volunteer': volunteer})
    def post(self,request,pid):
        if not request.user.is_authenticated:
            return redirect('/login-admin')
        volunteer=Volunteer.objects.get(id=pid)
